        ...
        <div>
            <Stack direction="row">
                <Stack flexGrow={1} height="300px" flexDirection="column">
                    {(project.length > 1) && (<Stack direction="row" style={{display: "flex", marginLeft: "5px"}}>
                        {project.map((projectButton, index) => (
                            <Button style={{
                                textTransform: "none",
                                backgroundColor: (projectAnalysisVersion === index) ? "#FFDD85" : "#FFF8E6",
                                borderRadius: "15px",
                                flexGrow: 1,
                                marginRight: "5px",
                                color: "#000000"
                            }} onClick={() => {
                                setProjectAnalysisVersion(index)
                            }}>
                                {project ? (project[index] ? Object.keys(project[index])[0] : "No name") : "No project"}
                            </Button>
                        ))}
                        <Button disabled={loadingVisible} style={{
                            ...
                            analyseUpdatedBranch(availableProjects[0].number, setProject, setLoadingVisible, setLoadingProblem);
                        }}>
                            {/*<SyncRoundedIcon/>*/}
                    ...
                    <Box variant="body1" p={2.25} color="#000000" bgcolor="#FFF8E6"
                        ...
                        <Stack direction="row">
                            <Stack direction="column" sx={{flexGrow: 1, justifyContent: "center"}}>
                                <Box style={{
                                    paddingLeft: "5px", paddingRight: "5px", marginBottom: "8px",
                                ...
                                </Typography>
                                {
                                    <Button type="submit" variant="outlined"
                                            sx={{
                                                ...
                                                width: (project.length < 2) ? "340px" : "165px"
                                            }} disableElevation size="large"
                                            onClick={() => setFeedbackDialogVisible(true)}>
                                        <ChatIcon sx={{marginRight: "15px"}}/>
                                        {(project.length < 2) && (<SyncRoundedIcon
                                            style={{height: "15px", position: "absolute", left: "2px"}}/>)}
                                        <Typography
                                            variant="body1">{(project.length > 1) ? "Feedback" : "Feedback & Analyse New Commits"}</Typography>
                                    </Button>
                            ...
                            <Box variant="body1" color="black" bgcolor="#F8F8F8"
                                ...
                                <Stack>
                                    ...
                                    <Typography style={{margin: "12px"}} variant="body" p={1}>
                                        You will now be redirected to Microsoft Forms. The current projects' analysis
                                        will
                                        be copied to your clipboard, please paste this into the indicated field. This
                                        will
                                        be anonymised and any irrelevant lines of code will be removed. The form will
                                        take
                                        about two minutes, thank you for taking the time to give feedback!
                                    </Typography>
                                    <Button style={{marginLeft: "12px", marginRight: "12px"}} startIcon={<ChatIcon/>}
                                            type="submit"
                                            variant="outlined" sx={{
                                        display: 'flex',
                                        color: "black",
                    ...
                    </Hidden>
                    {
                        (isSingleFile(project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]])) && (
                            <Content
                                inputErrors={project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]].issues}
                                description={project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]].issuesSummary}
                                title={[
                                    "This analysis will be lost after the page is closed, please use GitLab function to keep history"
                                ]}
                                headline={project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]].issuesSummaryTitle}
                                code={project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]].code}/>
                        )
                    }
                    {
                        (!isSingleFile(project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]])) && (
                    ...
                    <Hidden mdUp implementation="css">
                        <Commits style={{float: "right"}}
                                 commits={project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]].commits ? project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]].commits : [{
                                     authored_date: "Now",
                                     message: "Uploaded code via web editor",
                                     author_name: "You"
                                 }]}/>
                    </Hidden>
                </Stack>
                <Hidden mdDown implementation="css">
                    <Box sx={{width: "300px"}}>
                        <Commits
                            files={project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]]["files"]}
                            commits={project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]].commits ? project[projectAnalysisVersion][Object.keys(project[projectAnalysisVersion])[0]].commits : [{
                                authored_date: "Now",
                                message: "Uploaded code via web editor",
                                author_name: "You"
